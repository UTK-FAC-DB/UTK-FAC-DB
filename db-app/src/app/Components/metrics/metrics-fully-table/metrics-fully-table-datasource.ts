 ...
 */
export class MetricsFullyTableDataSource extends DataSource<FullyMetric> {
  private readonly _data: BehaviorSubject<FullyMetric[]> = new BehaviorSubject<FullyMetric[]>([]);

  /** Stream emitting render data to the table (depends on ordered data changes). */
  private readonly _renderData = new BehaviorSubject<FullyMetric[]>([]);
   ...
   */
  filteredData: FullyMetric[];
  multipleFilteredData: FullyMetric[];
  rangeAge: boolean = false;

  /** Array of data that should be rendered by the table, where each object represents one row. */
  get data() { return this._data.value; }
  set data(data: FullyMetric[]) { this._data.next(data); }
   ...
   */
  sortingDataAccessor: ((data: FullyMetric, sortHeaderId: string) => string|number) =
      (data: FullyMetric, sortHeaderId: string): string|number => {
   ...
   */
  sortData: ((data: FullyMetric[], sort: MatSort) => FullyMetric[]) = (data: FullyMetric[], sort: MatSort): FullyMetric[] => {
   ...
   */
  filterPredicate: ((data: FullyMetric, filter: string) => boolean) = (data: FullyMetric, filter: string): boolean => {
  ...
  constructor(private service: FullyService) {
    super();
    this.service.getItems();
    this.dataSub = this.service.getItemUpdateListener()
      .subscribe((data: FullyMetric[]) => {
        this.data = data;
      })
  ...
  _multipleFilterData(data: FullyMetric[]): FullyMetric[] {
    let count = 0;
    let check = 0;
   ...
   */
  _orderData(data: FullyMetric[]): FullyMetric[] {
   ...
   */
  _pageData(data: FullyMetric[]): FullyMetric[] {
