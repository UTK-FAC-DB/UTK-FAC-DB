...
oldDataFolder = currentPath + '/JSON_DATA_OLD'
newDataFolder = currentPath + '/JSON_DATA_NEW'
newFileData = []

# Loop through old data folder
print("Converting the following old files:")
for filename in os.listdir(oldDataFolder):
    print(filename[:-5])

    # Get old JSON
    with open(oldDataFolder + "/" + filename) as f:
      data = json.load(f)
...
# Saves new file
filePath = newDataFolder + '/' + filename[:-5] + '.json'
