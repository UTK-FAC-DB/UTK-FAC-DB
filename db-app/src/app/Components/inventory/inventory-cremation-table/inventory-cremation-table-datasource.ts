 ...
 */
export class InventoryCremationTableDataSource extends DataSource<CremationInventory> {
  private readonly _data: BehaviorSubject<CremationInventory[]> = new BehaviorSubject<CremationInventory[]>([]);

  /** Stream emitting render data to the table (depends on ordered data changes). */
  private readonly _renderData = new BehaviorSubject<CremationInventory[]>([]);
   ...
   */
  filteredData: CremationInventory[];
  multipleFilteredData: CremationInventory[];
  rangeAge: boolean = false;

  /** Array of data that should be rendered by the table, where each object represents one row. */
  get data() { return this._data.value; }
  set data(data: CremationInventory[]) { this._data.next(data); }
   ...
   */
  sortingDataAccessor: ((data: CremationInventory, sortHeaderId: string) => string|number) =
      (data: CremationInventory, sortHeaderId: string): string|number => {
   ...
   */
  sortData: ((data: CremationInventory[], sort: MatSort) => CremationInventory[]) = (data: CremationInventory[], sort: MatSort): CremationInventory[] => {
   ...
   */
  filterPredicate: ((data: CremationInventory, filter: string) => boolean) = (data: CremationInventory, filter: string): boolean => {
  ...
  constructor(private cremationService: CremationService) {
    super();
    this.cremationService.getItems();
    this.dataSub = this.cremationService.getItemUpdateListener()
      .subscribe((data: CremationInventory[]) => {
        this.data = data;
      })
   ...
   * as provided.
   */
  _filterData(data: CremationInventory[]) {
  ...
  _multipleFilterData(data: CremationInventory[]): CremationInventory[] {
    let count = 0;
    let check = 0;
   ...
   */
  _orderData(data: CremationInventory[]): CremationInventory[] {
   ...
   */
  _pageData(data: CremationInventory[]): CremationInventory[] {
