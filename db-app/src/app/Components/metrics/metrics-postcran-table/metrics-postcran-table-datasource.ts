 ...
 */
export class MetricsPostcranTableDataSource extends DataSource<PostcranMetric> {
  private readonly _data: BehaviorSubject<PostcranMetric[]> = new BehaviorSubject<PostcranMetric[]>([]);

  /** Stream emitting render data to the table (depends on ordered data changes). */
  private readonly _renderData = new BehaviorSubject<PostcranMetric[]>([]);
   ...
   */
  filteredData: PostcranMetric[];
  multipleFilteredData: PostcranMetric[];
  rangeAge: boolean = false;

  /** Array of data that should be rendered by the table, where each object represents one row. */
  get data() { return this._data.value; }
  set data(data: PostcranMetric[]) { this._data.next(data); }
   ...
   */
  sortingDataAccessor: ((data: PostcranMetric, sortHeaderId: string) => string|number) =
      (data: PostcranMetric, sortHeaderId: string): string|number => {
   ...
   */
  sortData: ((data: PostcranMetric[], sort: MatSort) => PostcranMetric[]) = (data: PostcranMetric[], sort: MatSort): PostcranMetric[] => {
   ...
   */
  filterPredicate: ((data: PostcranMetric, filter: string) => boolean) = (data: PostcranMetric, filter: string): boolean => {
  ...
  constructor(private service: PostcranService) {
    super();
    this.service.getItems();
    this.dataSub = this.service.getItemUpdateListener()
      .subscribe((data: PostcranMetric[]) => {
        this.data = data;
      })
   ...
   * as provided.
   */
  _filterData(data: PostcranMetric[]) {
  ...
  _multipleFilterData(data: PostcranMetric[]): PostcranMetric[] {
    let count = 0;
    let check = 0;
   ...
   */
  _orderData(data: PostcranMetric[]): PostcranMetric[] {
   ...
   */
  _pageData(data: PostcranMetric[]): PostcranMetric[] {
