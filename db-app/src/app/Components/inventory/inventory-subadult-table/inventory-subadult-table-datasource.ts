 ...
 */
export class InventorySubadultTableDataSource extends DataSource<SubadultInventory> {
  private readonly _data: BehaviorSubject<SubadultInventory[]> = new BehaviorSubject<SubadultInventory[]>([]);

  /** Stream emitting render data to the table (depends on ordered data changes). */
  private readonly _renderData = new BehaviorSubject<SubadultInventory[]>([]);
   ...
   */
  filteredData: SubadultInventory[];
  multipleFilteredData: SubadultInventory[];
  rangeAge: boolean = false;

  /** Array of data that should be rendered by the table, where each object represents one row. */
  get data() { return this._data.value; }
  set data(data: SubadultInventory[]) { this._data.next(data); }
   ...
   */
  sortingDataAccessor: ((data: SubadultInventory, sortHeaderId: string) => string|number) =
      (data: SubadultInventory, sortHeaderId: string): string|number => {
   ...
   */
  sortData: ((data: SubadultInventory[], sort: MatSort) => SubadultInventory[]) = (data: SubadultInventory[], sort: MatSort): SubadultInventory[] => {
   ...
   */
  filterPredicate: ((data: SubadultInventory, filter: string) => boolean) = (data: SubadultInventory, filter: string): boolean => {
  ...
  constructor(private service: SubadultService) {
    super();
    this.service.getItems();
    this.dataSub = this.service.getItemUpdateListener()
      .subscribe((data: SubadultInventory[]) => {
        this.data = data;
      })
   ...
   * as provided.
   */
  _filterData(data: SubadultInventory[]) {
  ...
  _multipleFilterData(data: SubadultInventory[]): SubadultInventory[] {
    let count = 0;
    let check = 0;
   ...
   */
  _orderData(data: SubadultInventory[]): SubadultInventory[] {
   ...
   */
  _pageData(data: SubadultInventory[]): SubadultInventory[] {
